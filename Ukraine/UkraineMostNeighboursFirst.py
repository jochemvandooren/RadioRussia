...
def calculate_towers(indices, solution, towerdict, startwith):
    if len(towerdict) == 27:
        return towerdict
    
    values = {}
    #count how many coloured neighbours the neighbours have of the last filled country
    for i in startwith:
        value=0
        for x in ukraine[i]:
            if x in towerdict:
                value+=1
        if i not in towerdict:
            values[i] = value

    #colour the neighbour that has the most filled in neighbours
    colourfirst = False
    if len(values) != 0:
        colourfirst = max(values.iteritems(), key=operator.itemgetter(1))[0]
    if not colourfirst:
        for i in startwith:
            indices.remove(i)
            indices.insert(0, i)
    else:
        indices.remove(colourfirst)
        indices.insert(0, colourfirst)
    ...
    for i in indices:
        if i not in solution:
            ...
            if towers2[0] in ['A', 'B', 'C', 'D']:
                ...
                result = calculate_towers(indices, solution + [i], towerdict, neighbours)
                if result != None:
                    return result
                del towerdict[i]
